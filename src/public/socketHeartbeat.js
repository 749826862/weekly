...
export default class socketHeartbeat {
  constructor({ options, callback }) {
    ...
    // 连接错误
    this.onerror = (e) => {
      console.log('websocket connection error:', e.code || '未知错误')
    };
    // 连接成功
    this.onopen = () => {
      console.log('websocket connection success')
      this.callback && this.callback(this.opts.pongMsg)
    };
    // 数据接收
    this.onmessage = (e) => {
      console.log('websocket accept data')
  ...
  socketSendData (msg) {
    ...
    try {
      if (msg !== this.opts.pongMsg && typeof JSON.parse(msg) === 'object') {
